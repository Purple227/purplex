					...
					<div class="table_wrapper">  <!-- Redundancy tag close -->
						...
						<table class="table is-bordered">  <!-- Table tag open -->
							...
							<tbody> <!-- tbody tag open -->
								<tr v-for="(subscriber, index) in searchQuery.length > 2 ? searchResult : subscribers" :key="index">
									<th class="has-text-success has-text-centered"> {{ index+1 }} </th>

									<td class=" has-text-centered"> {{ subscriber.email | truncate(0, 20)}} </td>

									<td class="has-text-centered"> {{ subscriber.created_at | format('D MMM YYYY - h:mm A') }} </td>

									<td class="has-text-centered">
										<span class="icon has-text-success">
											<i class="fas fa-trash" v-on:click="deleteData(subscriber.id, index)"> </i>
										</span>
						...
						<div class="notification is-primary has-text-centered" v-if="subscribers.length == 0">
...
export default {
	...
	methods: {
		...
		subscribersData(api) {
			let api_url = api || "/api/admin/subscriber"
			this.axios
		...
		searchData() {
			...
			if(this.searchQuery.length > 2) {
				axios.get('/api/admin/subscriber/table/search',{params: {search_query: this.searchQuery}}).then(response => {
		...
		deleteData(id, index) {
			...
			let api = '/api/admin/subscriber/' + id
			console.log(api)
